...
function getStrongNumber2_test(numberStr, lang = null, paramfirstLetter = null){
    ...
    .then((strong) => {
        ...
        arr_w.forEach((el,i,arr)=>{   
            ...
            if(el.includes('<df>Словарь Дворецкого:</df>')){
                ...
                el = `  <span class="btn btn_dvor" onclick="showHideDvor()">
                            Словарь Дворецкого: <img src="images/icon_razvernut.png">
                        </span>
                        <span class="dvor_block" style="display:none;">
        ...
        //Listener para Strong Heb
        if(typeof new_strongText !== 'undefined' && new_strongText.includes('sn2_s')){
            document.querySelectorAll('.text_strong S').forEach(el=>{                 
                el.addEventListener('click', function(){ 
                    getStrongNumber2(el.innerHTML);
                });    
            });
        }
        mySizeStrong();//altura de div_strong_body despues de meter div_strong_head
